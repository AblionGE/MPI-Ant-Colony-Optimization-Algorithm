...
OUTPUT_PARALLEL1_SPEEDUP="comparison1_speedup.txt"
OUTPUT_PARALLEL2_SPEEDUP="comparison2_speedup.txt"
OUTPUT_PARALLEL3_SPEEDUP="comparison3_speedup.txt"
...
do
  ...
  echo "$PARALLEL1ABSOLUTE" >> $OUTPUT_PARALLEL1_SPEEDUP
  echo "$PARALLEL1RELATIVE" >> $OUTPUT_PARALLEL1_SPEEDUP
  echo "$SERIALTIME $PARALLEL2TIME" >> $OUTPUT_PARALLEL2
  echo "$PARALLEL2ABSOLUTE" >> $OUTPUT_PARALLEL2_SPEEDUP
  echo "$PARALLEL2RELATIVE" >> $OUTPUT_PARALLEL2_SPEEDUP
  echo "$SERIALTIME $PARALLEL3TIME" >> $OUTPUT_PARALLEL3
  echo "$PARALLEL3ABSOLUTE" >> $OUTPUT_PARALLEL3_SPEEDUP
  echo "$PARALLEL3RELATIVE" >> $OUTPUT_PARALLEL3_SPEEDUP
